...
void input(){
	sum=0;
    scan(n);
    for(int i=1;i<=n;i++) scanl(a[i]),sum+=a[i];
}
...
int cnt[200005]={0}; 
bool check_composite(ll sm){
	for(ll i=2;i*i<=sm;i++){
		if(sm%i==0) return true;
	}
	return false;
}
int main(){
   ...
   // #endif
    ...
    w(t){
      ...
      int ind=-1;
      for(int i=0;i<=n;i++){
         if(check_composite(sum-a[i]) and (sum-a[i])>ans){
         	ans=(sum-a[i]);
         	ind=i;
         }
      }
